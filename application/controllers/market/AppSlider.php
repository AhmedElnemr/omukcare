<?php
class AppSlider extends MY_Controller{
	private $folder = "app_slider";
	private $con    = "app-slider";
	public function __construct(){
		parent::__construct();
		$this->load->model('Slider_model','Cmodel');
	...
	public  function index(){
		...
		$data["data_table"] = $this->Cmodel->get_many_by(["type"=>'app']);
		$data['metadiscription'] = $data['metakeyword'] = $data['title'] = 'بنر التطبيق ';
	...
	public  function create(){
		...
		if ($this->input->post('INSERT') == "INSERT") {
			...
			foreach ($Tdata as $key=>$val){
				...
				$this->Slider_trans_model->insert($data);
			}*/
			//----------------------------------------------
			$this->message('s');
	...
	public  function update($id){
		...
		if ($this->input->post('UPDTATE') == "UPDTATE") {
			...
			/*$this->Slider_trans_model->delete_by(["slider_id"=>$id]);
			$Tdata = $this->input->post('Pdata');
			foreach ($Tdata as $key=>$val){
				$data["slider_id"] = $id;
				$data["lang"] = $key;
