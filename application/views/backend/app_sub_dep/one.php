...
<div class="m-portlet__body">
	...
	<div class="form-group m-form__group row">
		<div class="col-lg-6">
			...
			<select class="form-control" name="Tdata[perant_id]"  data-validation="required">
				<option value="">اختر </option>
				<?php if (isset($main) && !empty($main)): ?>
					<?php foreach ($main as $one): ?>
						<option value="<?=$one->id?>" <?=($out["perant_id"] == $one->id)?"selected":""?> >
							<?=$one->word->title?></option>
					<?php endforeach ?>
				<?php endif ?>
			</select>
		</div>
