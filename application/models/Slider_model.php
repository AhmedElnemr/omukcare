...
class Slider_model extends MY_Model {
    ...
    protected  function getTrans($row){
        if (!empty($row)) {
            ...
            if (is_object($row)) {
                if (isset($row->id)) {
                    $row->word = $this->get_trans_object("slider_trans", ["lang" => $lang, "slider_id" => $row->id]);
                } else {
                    $row->word = $this->get_trans_object("slider_trans", ["lang" => $lang, "slider_id" => 0]);
                }
            } else {
                if (isset($row['id'])) {
                    $row["word"] = $this->get_trans_object("slider_trans", ["lang" => $lang, "slider_id" => $row['id']]);
