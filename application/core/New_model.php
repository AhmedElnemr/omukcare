...
class New_model extends CI_Model
{
	...
	*/
	...
	public function get_filds(){
		$query = $this->_database->query("select * from ".$this->_table);
	...
	public function get_trans_filds(){
		$query = $this->_database->query("select * from ".$this->_trans_table);
		$field_array = $query->list_fields();
		foreach ($field_array as $key=>$value){
			$out[$value]='';
		}
	...
	public function get_with_in($data,$with){
		$this->with_meny($with);
		foreach ($data as $key => &$row)
		{
			$row = $this->trigger('after_get', $row, ($key == count($data) - 1));
		}
		return $data;
	}

	public function with_meny($relationship)
	{
		$this->_with = $relationship;
	...
	public function get_ids_by($fild,$where){
		$this->_database->select($fild);
		$this->_database->from($this->_table);
	 ...
	 * Fetch an array of records based on an arbitrary WHERE call.
	 */
	public function get_many_by_in($where,$key,$InArray)
	{
		$this->_database->where_in($key,$InArray);
		$this->_database->where($where);
		return $this->get_all();
	}
	/**
	 * Fetch an array of records based on an arbitrary WHERE call.
	 */
	public function get_many_in($key,$InArray)
	{
		$this->_database->where_in($key,$InArray);
		return $this->get_all();
	}
	...
	*/
	public function trans($lang){
		...
		if (!in_array('get_translate', $this->after_get))
		{
			$this->after_get[] = 'get_translate';
		}

		return $this;
	}

	public function get_translate($row)
	{
		if (empty($row))
		{
			return $row;
		...
		foreach ($this->translate_to as $key => $value) {
			if (is_string($value))
			{
				$relationship = $value;
				$options = array( 'primary_key' => $value . '_id', 'model' => $value . '_model' );
			...
			$whereCase = [
				$options['primary_key_fild']=>$row->{$options['primary_key']},
				$options["language_fild"]=>$this->_lang
			];
			if (is_object($row))
			{
				$row->{$relationship} = $this->{$relationship . '_model'}->get_trnas_by($whereCase);
			}
			else
			{
				$row[$relationship]  = $this->{$relationship . '_model'}->get_trnas_by($whereCase);
	...
	public function get_trans_object($table,$where){
		...
		if ($query->num_rows() > 0) {
			...
			$field_array = $this->_database->query("select * from ".$table)->list_fields();
			foreach ($field_array as $key=>$value){
				$out[$value]='';
			}
			return (object)$out;
	...
	public function found_object($table,$where){
		$this->_database->select('*');
		$this->_database->from($table);
		$this->_database->where($where);
		$query = $this->db->get();
	...
	*/
	public  function getById($id){
		...
		if(!empty($this->trans_lang)){
			foreach ($this->trans_lang as $key=>$value) {
				$data[$value] = $this->getSubTrans($id, $value);
			}
		}
		return $data ;
	}
	...
	public  function getSubTrans($id,$lang){
		$this->_database->select('*');
		$this->_database->from($this->_trans_table);
		$this->_database->where($this->trans_key,$id);
		$this->_database->where('lang',$lang);
		$query = $this->db->get();
		if ($query->num_rows() > 0) {
			$data = $query->row_array();
			return $data;
		}
		return $this->get_trans_filds();
	}

	public function with_trans(){
		if (!in_array('getTrans', $this->after_get))
		{
			$this->after_get[] = 'getTrans';
		}
		return $this;
	}


	public function getTrans($row){
		$lang = getLocalize();
		if (!empty($row)) {
			if (is_object($row)) {
				if (isset($row->{$this->primary_key})) {
					$row->word = $this->getTransObject($lang,$row->{$this->primary_key} );
				} else {
					$row->word = $this->getTransObject($lang,$row->{$this->primary_key});
				}
				// end if
	...
	public function getTransObject($lang,$id){
		$this->_database->select('*');
		$this->_database->from($this->_trans_table);
		$this->_database->where("lang", $lang);
		$this->_database->where( $this->trans_key,$id);
		...
		if ($query->num_rows() > 0) {
			...
			$field_array = $this->_database->query("select * from ".$this->_trans_table)->list_fields();
			foreach ($field_array as $key=>$value){
				$out[$value]='';
			}
			return (object)$out;
