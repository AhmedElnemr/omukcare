...
class Area extends MY_Controller{
	private $folder = "area";
	private $con    = "admin-area";
	public function __construct(){
		parent::__construct();
		$this->load->model('Areas_model','Cmodel');
		$this->load->model('Areas_trans_model');
	...
	public  function index(){
		...
		$data['metadiscription'] = $data['metakeyword'] = $data['title'] = 'المناطق ';
	...
	public  function create(){
		...
		if ($this->input->post('INSERT') == "INSERT") {
			...
			foreach ($Tdata as $key=>$val){
				...
				$this->Areas_trans_model->insert($data);
			}
			//----------------------------------------------
			$this->message('s');
	...
	public  function update($id){
		...
		if ($this->input->post('UPDTATE') == "UPDTATE") {
			...
			$this->Areas_trans_model->delete_by(["area_id"=>$id]);
			$Tdata = $this->input->post('Pdata');
			foreach ($Tdata as $key=>$val){
				$data["area_id"] = $id;
				$data["lang"] = $key;
