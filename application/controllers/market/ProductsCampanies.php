<?php
class ProductsCampanies extends MY_Controller{
	private $folder = "app_product_campanies";
	private $con    = "app-product-campanies";
	public function __construct(){
		parent::__construct();
		$this->load->model('market/Products_campanies_model','Cmodel');
		$this->load->model('market/Products_campanies_trans_model',"Tmodel");
	}

	public  function index(){

		$data["data_table"] = $this->Cmodel->get_all();
		$data['metadiscription'] = $data['metakeyword'] = $data['title'] = 'الشركات ';
	...
	public  function update($id){
		...
		if ($this->input->post('UPDTATE') == "UPDTATE") {
			...
			$this->Tmodel->delete_by(["campany_id"=>$id]);
			$Tdata = $this->input->post('Pdata');
			foreach ($Tdata as $key=>$val){
				$data["campany_id"] = $id;
				$data["lang"] = $key;
